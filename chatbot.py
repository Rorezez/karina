...
# Create a dictionary to store the "trash" conversation history
trash_conversations = {}
...
def generate_response_chat(message_list):
    ...
    if "inget" in last_message["content"] and faiss_index:
        ...
        print("After update:", message_list)
...
def send_response_in_sentences(bot, message, response_text):
    sentences = response_text.split('.')
    for sentence in sentences:
        if sentence.strip():  # This checks if the sentence is not empty
                bot.send_message(chat_id=message.chat.id, text=sentence.strip())
...
def conversation_tracking(text_message, user_id):
    ...
    # If there are more than 10 conversations, move the oldest one to the trash
    if len(user_conversations['conversations']) > 10:
        trash_conversations[user_id] = {
            'conversations': user_conversations['conversations'][:-10],
            'responses': user_conversations['responses'][:-10]
        }
    ...
    # After storing the updated conversations and responses, print them for logging
    print("Current conversation history for user", user_id, ":")
    print(conversations[user_id])
    # If this user has a trash conversation history, print it
    if user_id in trash_conversations:
        print(f"Trash conversation history for user {user_id}:")
        print(trash_conversations[user_id])
...
def start(message):
    if message.text.startswith("/help"):
        ...
        bot.send_message(message, "Just start chatting to the AI or enter /help for other commands")
...
def echo_message(message):
    ...
    if message.text == '/clear':
        ...
        return
    # Send 'typing...' action before generating the response
    bot.send_chat_action(message.chat.id, 'typing')
    
    response = conversation_tracking(message.text, user_id)

      # Send the response in sentences
    send_response_in_sentences(bot, message, response)
